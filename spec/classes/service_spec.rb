    ...
    context "with facter 3.7 on #{os}" do
      ...
      context 'with defaults' do
        ...
        when 'RedHat'
          ...
          when '7'
            it do
              is_expected.to contain_file('/usr/lib/tmpfiles.d/grafana.conf')
                .with_content('d /var/run/grafana 0755 grafana grafana')
            end

            it do
              is_expected.to contain_exec('Create Systemd Temp Files')
                .with_refreshonly(true)
            end
          else
            it do
              is_expected.not_to contain_file('/usr/lib/tmpfiles.d/grafana.conf')
            end

            it do
              is_expected.not_to contain_exec('Create Systemd Temp Files')
            end
          end
